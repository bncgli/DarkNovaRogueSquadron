...
func _populate_default_ship_drive_files() -> void:
	...
	if bp_files.size() > 0:
		...
		# File di testo informativi e di sistema se non inclusi nella blueprint
		...
		if not FileAccess.file_exists("user://files/Ship Drive/systems/ship_blueprint.dat"):
