...
# Stato sincronizzato del Duct Drone (delegato a drone_manager, unica fonte di verità)
var duct_drone_pos: Vector2:
	get: return drone_manager.pos
	set(value): drone_manager.pos = value
var duct_drone_heading: float:
	get: return drone_manager.heading
	set(value): drone_manager.heading = value
var duct_drone_speed: float:
	get: return drone_manager.speed
	set(value): drone_manager.speed = value
var duct_drone_battery: float:
	get: return drone_manager.battery
	set(value): drone_manager.battery = value
var duct_drone_lights: bool:
	get: return drone_manager.lights
	set(value): drone_manager.lights = value
var duct_drone_scan_active: bool:
	get: return drone_manager.scan_active
	set(value): drone_manager.scan_active = value
var duct_drone_scan_radius: float:
	get: return drone_manager.scan_radius
	set(value): drone_manager.scan_radius = value

var duct_drone_linear_input: float:
	get: return drone_manager.linear_input
	set(value): drone_manager.linear_input = value
var duct_drone_angular_input: float:
	get: return drone_manager.angular_input
	set(value): drone_manager.angular_input = value
var duct_drone_speed_mult: float:
	get: return drone_manager.speed_mult
	set(value): drone_manager.speed_mult = value

# Danni strutturali e sistemici alla nave (delegati a damage_manager, unica fonte di verità)
var ship_damages: Array[ShipDamageRuntimeState]:
	get: return damage_manager.active_damages
var is_duct_drone_repairing: bool:
	get: return drone_manager.is_repairing
	set(value): drone_manager.is_repairing = value
var repairing_damage_id: String:
	get: return drone_manager.repairing_damage_id
	set(value): drone_manager.repairing_damage_id = value
...
## Blocca/sblocca il movimento cinematico della nave (es. durante l'attracco a una stazione)
func lock_ship_movement(locked: bool) -> void:
	var ship := get_spaceship()
	if ship and is_instance_valid(ship):
		ship.set_movement_locked(locked)

## Ritorna il transform globale reale della nave
func get_ship_transform() -> Transform3D:
	var ship := get_spaceship()
	if ship and is_instance_valid(ship):
		return ship.global_transform if ship.is_inside_tree() else ship.transform
...
func set_duct_drone_inputs(linear_in: float, angular_in: float, speed_mult: float = 1.0) -> void:
	...
	drone_manager.set_inputs(linear_in, angular_in, speed_mult)
...
func reset_duct_drone() -> void:
	...
	drone_manager.reset(get_drone_spawn_pos(), get_drone_spawn_heading())
...
func set_duct_drone_lights(enabled: bool) -> void:
	...
	if nm and nm.get("is_connected_to_network") and not nm.get("is_host"):
		...
		drone_manager.set_lights(enabled)
		return
	drone_manager.set_lights(enabled)
...
func apply_synced_duct_drone_state(pos: Vector2, heading: float, speed: float, battery: float, lights: bool, scan_active: bool, scan_radius: float) -> void:
	drone_manager.apply_synced_state(pos, heading, speed, battery, lights, scan_active, scan_radius)

func set_duct_drone_config(cfg: Dictionary) -> void:
	drone_manager.set_config(cfg)

func get_duct_drone_config() -> Dictionary:
	return drone_manager.get_config()

## Delega l'intera simulazione fisica del tick (movimento, batteria, rilevamento danni,
## avanzamento riparazione) a DuctDroneManager/ShipDamageManager e gestisce solo la
## sincronizzazione di rete conseguente.
func _update_duct_drone_physics(delta: float) -> void:
	var damages_changed := drone_manager.update_physics(delta, damage_manager)
...
func configure_initial_station_spawn() -> void:
	...
	if station_data.is_empty():
		...
		if grid_mgr and grid_mgr.has_method("get_starting_station"):
			var start_st: CelestialBodyData = grid_mgr.get_starting_station()
			if start_st != null:
				station_data = start_st.to_dict()
...
## Segnala un allarme di sistema (es. surriscaldamento) tramite le notifiche diegetiche esistenti
func report_system_alert(message: String) -> void:
	_send_spawn_notification(message)
...
func _rpc_client_duct_drone_input(lin_in: float, ang_in: float, speed_mult: float) -> void:
	...
	if nm == null or not nm.get("is_host"):
		return
	drone_manager.set_inputs(lin_in, ang_in, speed_mult)
...
func get_ship_damages() -> Array[ShipDamageRuntimeState]:
	return damage_manager.get_damages()

func get_active_ship_damages() -> Array[ShipDamageRuntimeState]:
	return damage_manager.get_active_damages()

func get_damage_by_id(dmg_id: String) -> ShipDamageRuntimeState:
	return damage_manager.get_damage_by_id(dmg_id)

func get_adjacent_damage(pos: Vector2, max_dist: float = 38.0) -> ShipDamageRuntimeState:
	return damage_manager.get_adjacent_damage(pos, max_dist)

## Determina tipo/posizione/settore (se non forniti) in base alla ShipBlueprint attiva e
## delega la creazione effettiva del danno a ShipDamageManager.
func spawn_ship_damage(type: String = "", pos: Vector2 = Vector2.ZERO, sector_name: String = "", duration: float = 0.0) -> ShipDamageRuntimeState:
	...
	var dmg := damage_manager.spawn_damage(type, pos, sector_name, duration)
...
func generate_initial_ship_damages(count: int = 4) -> void:
	damage_manager.reset_for_generation()
...
func clear_ship_damages() -> void:
	drone_manager.is_repairing = false
	drone_manager.repairing_damage_id = ""
...
func start_duct_drone_repair(damage_id: String) -> void:
	...
	if nm and nm.get("is_connected_to_network") and not nm.get("is_host"):
		...
		drone_manager.is_repairing = true
		drone_manager.repairing_damage_id = damage_id
		return
	
	var dmg: ShipDamageRuntimeState = damage_manager.get_damage_by_id(damage_id)
	if dmg == null or duct_drone_battery <= 0.0:
		return
	if dmg.repaired:
		return
	
	var d: float = duct_drone_pos.distance_to(dmg.pos)
	var repair_rng: float = float(drone_manager.active_config.get("repair_range", 42.0))
	if d > repair_rng:
		return
	
	var progress: float = dmg.repair_progress
	drone_manager.start_repair(damage_id, progress)
...
func is_duct_drone_repairing_active() -> bool:
	return drone_manager.is_repairing

func get_repairing_damage_id() -> String:
	return drone_manager.repairing_damage_id

# --- DAMAGE & REPAIR RPC HANDLERS ---

@rpc("authority", "call_remote", "reliable")
func _rpc_sync_ship_damages(p_damages: Array) -> void:
	damage_manager.sync_from_snapshot(p_damages)

@rpc("authority", "call_remote", "unreliable_ordered")
func _rpc_sync_repair_state(is_repairing: bool, damage_id: String, progress: float) -> void:
	drone_manager.is_repairing = is_repairing
	drone_manager.repairing_damage_id = damage_id
	damage_manager.set_repair_progress(damage_id, progress)
...
func get_weapon_targets() -> Array[Dictionary]:
		...
		for child in asteroids:
			if child and is_instance_valid(child) and child.visible:
				...
				targets.append({
					...
					"local_velocity": ship_basis.inverse() * (child.rotation_speed if "rotation_speed" in child else Vector3(0.1, 0.0, 0.2)),
		...
		for foe in hostiles:
			if foe and is_instance_valid(foe) and foe.visible:
				...
				targets.append({
					...
					"distance": dist,
					"velocity": f_vel,
					"local_velocity": ship_basis.inverse() * f_vel,
					"bearing_deg": bearing_deg,
					"elevation_deg": elevation_deg,
					"type": "SHIP_HOSTILE",
	...
	if targets.is_empty():
		...
		for dt in default_targets:
			...
			targets.append({
				...
				"distance": dist,
				"velocity": dt["vel"],
				"local_velocity": ship_basis.inverse() * dt["vel"],
				"bearing_deg": bearing_deg,
				"elevation_deg": elevation_deg,
				"type": "CONTACT",
	...
	for p in active_probes:
		...
		targets.append({
			...
			"local_velocity": ship_basis.inverse() * p.get("velocity", Vector3.ZERO),
			"bearing_deg": bearing_deg,
			"elevation_deg": elevation_deg,
			"type": "PROBE",
			"threat_level": "FRIENDLY"
	...
	if _space_scene_instance and is_instance_valid(_space_scene_instance):
		...
		for child in asteroids:
			if child and is_instance_valid(child) and child.visible:
				...
				var max_scale := maxf(child.scale.x, maxf(child.scale.y, child.scale.z)) if child is Node3D else 1.0
				var r_m: float = 4.0 * max_scale
				if child.has_node("CollisionShape3D"):
					var cs := child.get_node("CollisionShape3D") as CollisionShape3D
					if cs and cs.shape is SphereShape3D:
						r_m = cs.shape.radius * max_scale
				elif "radius_m" in child and float(child.get("radius_m")) > 0.0:
					r_m = float(child.get("radius_m"))
		...
		for lrd in long_range_defaults:
			if not existing_ids.has(lrd["id"]):
				...
				entities.append({
					...
					"distance": dist,
					"velocity": lrd["vel"],
					"local_velocity": ship_basis.inverse() * lrd["vel"],
					"bearing_deg": bearing_deg,
					"elevation_deg": elevation_deg,
					"type": lrd["type"],
					"iff_tag": lrd["iff_tag"],
