...
- Creare il comando `datread <file.dat>`: apre il file `.dat` specificato direttamente nell'applicazione `Text Editor` di GodotOS, bypassando la schermata di blocco per i file protetti e consentendo la modifica diegetica dei parametri di configurazione.

## Criteri di Accettazione
- [ ] **Comando Terminale `worm`**:
  - [ ] Sintassi: `worm <nome_cartella>` (o percorso relativo).
  - [ ] Recupera la password reale protetta da `FolderPasswordManager`.
  - [ ] Gestisce turni di tentativi interattivi mostrando feedback visivo diegetico (es. `[MATCH: 2 ESATTI, 1 FUORI POSTO]`).
  - [ ] Al completamento con successo, la password della cartella viene sbloccata/rivelata.
- [ ] **Comando Terminale `decript`**:
  - [ ] Sintassi: `decript <file.dat>`.
  - [ ] Visualizza la frase *"the quick brown fox jumps over the lazy dog"* criptata con la chiave del file.
  - [ ] Inserendo i caratteri corretti della chiave, la stringa si decifra progressivamente a schermo fino alla soluzione.
- [ ] **Comando Terminale `datread`**:
  - [ ] Sintassi: `datread <file.dat>`.
  - [ ] Verifica l'esistenza del file `.dat` e lo apre nel `Text Editor` con permessi di visualizzazione e scrittura abilitati.
...
- **File coinvolti**:
  ...
  - `Applications/Terminal/commands/datread_command.gd`: Nuovo script che estende `TerminalCommand`.
  ...
  - `Scenes/Window/Text Editor/text_editor.gd`: Metodo `open_dat_file(path: String)` per consentire la lettura autorizzata da `datread`.
...
3. **Implementazione di `Applications/Terminal/commands/datread_command.gd`**:
   - Creare la classe derivata da `TerminalCommand`:
     ```gdscript
     extends TerminalCommand
     func _init() -> void: call_name = "datread"
     ```
   - In `execute(terminal: Terminal, args: Array[String])`:
     - Validare l'argomento `args[0]` verificando che termini con `.dat`.
...
4. **Adattamento `Scenes/Window/Text Editor/text_editor.gd`**:
   ...
   - Modificare `populate_text(path: String)`:
     - Se `path.ends_with(".dat")`:
       ...
       - Altrimenti: mantenere il blocco con messaggio di errore informativo che suggerisce l'uso del comando `datread`.
