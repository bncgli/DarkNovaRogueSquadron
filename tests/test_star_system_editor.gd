...
func test_star_system_data_serialization() -> void:
	...
	assert(b.get("name") == "Planet Test", "Il nome del corpo celeste deve corrispondere.")
	
	var d := sys.to_dict()
	assert(d.get("system_id") == "SYS-TEST-01", "Il system_id nel dict deve corrispondere.")
...
func test_star_system_canvas_interactions() -> void:
	...
	assert(hit.get("id") == "BODY_CENTER", "Il hit test del canvas deve trovare l'entità al centro.")
