...
func _setup_ui() -> void:
	...
	tool_selector.add_item("Add Device", ShipBlueprintCanvas.ToolMode.ADD_DEVICE)
	tool_selector.add_item("Add Junction", ShipBlueprintCanvas.ToolMode.ADD_JUNCTION)
	tool_selector.add_item("Add Conduit", ShipBlueprintCanvas.ToolMode.ADD_CONDUIT)
	tool_selector.add_item("Add Damage", ShipBlueprintCanvas.ToolMode.ADD_DAMAGE)
	...
	_add_layer_checkbox("Power Grid", "show_power_grid")
	_add_layer_checkbox("Damages", "show_damages")
...
func _rebuild_inspector(type: String, id: String, data: Dictionary) -> void:
	...
	if type.is_empty():
		...
		inspector_container.add_child(lbl)
		return
...
func _update_element_property(type: String, id: String, key: String, value: Variant) -> void:
	...
	match type:
		...
		"junction": elem = current_blueprint.get_junction_by_id(id)
		"conduit": elem = current_blueprint.get_conduit_by_id(id)
...
func _on_save_pressed() -> void:
	var dir = "user://files/Terminal Drive/Programs/ShipBuilder"
	if not DirAccess.dir_exists_absolute(dir):
		DirAccess.make_dir_recursive_absolute(dir)
	var path = dir + "/custom_ship.tres"
	var err = ResourceSaver.save(current_blueprint, path)
	if err == OK:
		_update_status("Saved to ShipBuilder/custom_ship.tres")
	else:
		_update_status("Save failed: %d" % err)

func _on_load_pressed() -> void:
	var path = "user://files/Terminal Drive/Programs/ShipBuilder/custom_ship.tres"
	if FileAccess.file_exists(path):
		var res = load(path)
		if res is ShipBlueprint:
			current_blueprint = res
			canvas.blueprint = current_blueprint
			_update_status("Loaded custom_ship.tres")
	else:
...
func _on_software_pressed() -> void:
	...
	market_vb.add_child(Label.new()); market_vb.get_child(0).text = "Marketplace"
	...
	var btn_rem = Button.new(); btn_rem.text = " << "
	...
	inst_vb.add_child(Label.new()); inst_vb.get_child(0).text = "Installed Software"
